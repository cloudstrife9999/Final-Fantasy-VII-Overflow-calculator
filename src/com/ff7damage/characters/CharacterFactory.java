...
public class CharacterFactory {
	public static CharacterInterface createCharacter(byte code, int level, WeaponInterface weapon, int strength, byte[] extra,
			byte[] anotherExtra, boolean berserked, boolean inFrog, boolean inMini, Row row) {
		
		switch(code) {
		case 0x00:
		{
			int currentHp = ByteBuffer.allocate(4).put(extra).getInt(0);
			int maxHp = ByteBuffer.allocate(4).put(anotherExtra).getInt(0);
		...
		case 0x02:
		{
			int limitLevel = ByteBuffer.allocate(4).put(extra).getInt(0);
			int limitGauge = ByteBuffer.allocate(4).put(anotherExtra).getInt(0);
		...
		case 0x04:
		{
			int currentMp = ByteBuffer.allocate(4).put(extra).getInt(0);
			int maxMp = ByteBuffer.allocate(4).put(anotherExtra).getInt(0);
		...
		case 0x07:
		{
			int kills = ByteBuffer.allocate(4).put(extra).getInt(0);
